    ...
    class Klaviyo_WP_Meta_Sync_Admin_Usermeta {
        ...
        public function __construct() {
           ...
           self::metadata_to_sync();
			...
			add_action( 'manage_users_extra_tablenav', [ self::class, 'add_sync_all_users_extra_tablenav' ], 10, 1 );
			add_action( 'admin_init', [ self::class, 'handle_sync_all_users_action' ] );
            add_action( 'admin_notices', [ self::class, 'bulk_actions_admin_notice' ] );
            add_action( 'wp_ajax_bulk_sync_progress', [ self::class, 'bulk_sync_progress_callback' ] );
        }
        ...
        public static function handle_bulk_action( $redirect_to, $doaction, $user_ids ) {
            ...
            foreach ( $batches as $batch_index => $batch_user_ids ) {
                ...
                wp_schedule_single_event( time() + ( $batch_index * 60 ), 'klaviyo_sync_batch_event', [ $batch_id ] );
            }
        ...
        public static function handle_batch_event( $batch_id ) {       
            ...
            foreach ( $users as $user ) {
                ...
                if ( isset( $user_ids[ $user_id ] ) && $user_ids[ $user_id ] === 'completed' ) {
                    continue;
                }
...
;
        ...
        public static function set_users_to_processing_state( $user_ids ) {
            ...
            $delete_params = array_merge( [ '_klaviyo_sync_status' ], $user_ids );
        ...
        public static function bulk_sync_progress_callback() {;
            ...
            if ( ! is_array( $user_ids ) ) {
                wp_send_json_error( [ 'message' => 'No users in progress.' ] );
            }
            ...
            foreach ( $user_ids as $user_id => $status ) {
                if ( 'completed' === $status ) {
                    ...
                    $failed_users[] = [
                        'user_id'    => $user_id,
                        'error_message' => __( 'Failed to sync!.', 'originality-ai' ),
                    ];
